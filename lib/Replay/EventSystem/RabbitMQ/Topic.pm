...
sub _build_topic_name {     ## no critic (ProhibitUnusedPrivateSubroutines)
    my ($self) = @_;
    if (!defined $self->purpose) {
        confess q(No purpose);
    }
...
=head1 SYNOPSIS
    ...
    my $topic = Replay::EventSystem::RabbitMQ::Topic->new(
        rabbit        => $self,
...
Used to open a rabbitmq topic that will provide a message transmission channel
to a particular exchange on the RabbitMQ server
