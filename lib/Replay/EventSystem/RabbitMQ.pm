...
has unique => (is => 'ro', isa => 'Num', required => 0, default => 0);
has connection => (
    is      => 'ro',
...
sub _build_rabbit {    ## no critic (ProhibitUnusedPrivateSubroutines)
    ...
    my $rabbit;
    try {
        $rabbit = Replay::EventSystem::RabbitMQ::Connection->instance;
    }
    catch {
...
sub emit {
    ...
    if ('HASH' eq ref $message) {
        confess 'Not a good hash message' . Dumper $message
            if !defined $message->{MessageType};
    }
    if (!blessed $message) {
    ...
    if (!$message->does('Replay::Envelope')) {
...
sub done {
    ...
    #    return $self->connection->_clear_instance;
}
...
Replay::EventSystem::RabbitMQ - RabbitMQ Event implimentation for Replay
...
If you were to instantiate it independently, it would look like the SYNOPSIS
...
Being part of the Replay event framework, the API this impliments is probably
not going to be used anywehre else.

=head1 DEPENDENCIES

The Replay bitemporal framework
...
The module seems to confuse its channels, causing messages to be misprocessed
once more than a trivial amount of messages are being exchanged.  It is 
suspected that this is a problem with the Net::RabbitMQ / libRabbitMQ support
libraries. Patches welcome.
